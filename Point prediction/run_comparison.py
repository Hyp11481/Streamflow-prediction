...
class LSTMModel(nn.Module):
    def __init__(self, 
        ...
        self.fc = nn.Linear(hidden_dim, 7)  # 7个提前期预测
...
class CNNLSTMModel(nn.Module):
    def __init__(self, 
        ...
        self.dropout = nn.Dropout(dropout_rate)
        self.fc = nn.Linear(lstm_hidden, 7)  # 7个提前期预测
...
class TransformerModel(nn.Module):
    def __init__(self, 
        ...
        # 输出层
        self.dropout = nn.Dropout(dropout_rate)
        self.fc = nn.Linear(d_model, 7)  # 7个提前期预测
...
class MLPModel(nn.Module):
    def __init__(self, 
        ...
        self.fc3 = nn.Linear(hidden_dim // 2, 7)  # 7个提前期预测
...
def process_station_with_model_comparison(station_name, upstream_stations, 
    ...
    不使用NRBO优化的VMD分解或任何降噪方法，直接使用原始数据。
    
    Args:
        station_name: 目标站点名称
        upstream_stations: 上游站点列表
        model_types: 要对比的模型类型列表，默认为['tcn', 'lstm', 'gru', 'cnn_lstm', 'transformer', 'mlp']
        nrbo_population: NRBO算法种群大小（未使用）
        nrbo_iterations: NRBO算法迭代次数（未使用）
    ...
    try:
        # 创建结果保存目录
        station_dir = f"results_{station_name}_model_comparison_no_denoise"
        ...
        for idx, (days, suffix) in enumerate(prediction_periods):
            try:
                ...
                plt.plot(dates, actual_values[:, idx], 'k-', label='实际值', linewidth=2)
                
                # 为每种模型绘制预测值
                colors = ['#2878B5', '#F8766D', '#00BA38', '#C77CFF', '#FFA500', '#8B4513']
                for i, model_type in enumerate(model_types):
                    plt.plot(dates, model_predictions[model_type][:, idx], 
                            linestyle='--', color=colors[i % len(colors)], 
                            label=f'{model_type.upper()}预测值', linewidth=1.5, alpha=0.8)
                
                plt.title(f'{station_name}站点各模型{days}天预测对比', fontsize=16)
                ...
                for col in metrics_df.columns:
                    # 对RMSE, MAE, sMAPE是越小越好
                    if col in ['RMSE', 'MAE', 'sMAPE']:
                ...
                # 创建多个子图
                metrics_to_plot = ['RMSE', 'R2', 'NSE', 'sMAPE']
        ...
        summary = {
            ...
            "最佳模型": {
                ...
                "总体指标": {
                    ...
                    for metric in ['RMSE', 'MAE', 'R2', 'NSE', 'sMAPE']
                }
            },
            "预测时间范围": {
            ...
            "数据处理说明": "未使用NRBO优化的VMD分解或任何降噪方法，直接使用原始径流量数据"
        }
        ...
        logger.info(f"总体sMAPE: {overall_df.loc[best_model, 'sMAPE']:.2f}%")
...
def compare_models_on_selected_stations(stations_to_compare=None, model_types=None, nrbo_population=30, nrbo_iterations=30):
    """对选定的站点运行模型对比试验
