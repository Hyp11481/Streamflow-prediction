...
def evaluate_interval_forecasts(station_name, alpha=0.05):
    """
    评估单个站点的区间预测效果，生成综合报告
    ...
    """
    # 定义结果目录
    results_dir = f"results_{station_name}_optimized"
    
    if not os.path.exists(results_dir):
        logger.error(f"站点{station_name}的结果目录不存在")
        return
    
    # 读取预测指标
    metrics_file = os.path.join(results_dir, f"{station_name}_metrics.json")
    if not os.path.exists(metrics_file):
        logger.error(f"站点{station_name}的指标文件不存在")
        return
    ...
    # 创建区间预测评估报告目录
    interval_dir = os.path.join(results_dir, "interval_forecasts")
    ...
    for days, suffix in prediction_periods:
        # 读取预测结果CSV
        csv_file = os.path.join(results_dir, f"{station_name}_{suffix}_predictions.csv")
        if not os.path.exists(csv_file):
            logger.warning(f"站点{station_name}的{days}天预测结果文件不存在")
            continue
        ...
        # 获取PICP和PINAW
        if f"{days}天预测" in metrics["各提前期评估指标"]:
            period_metrics = metrics["各提前期评估指标"][f"{days}天预测"]
            picp = period_metrics.get("PICP", 0)
            pinaw = period_metrics.get("PINAW", 0)
    ...
    ax2.set_title(f'{station_name}站点各预测期PINAW (预测区间归一化平均宽度)')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(interval_dir, f"{station_name}_interval_metrics.png"), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    summary = {
        ...
        "预测区间显著性水平": alpha,
        "目标覆盖率": 1-alpha,
        "平均PICP": np.mean(picp_values),
        "最大PICP": np.max(picp_values),
        "最小PICP": np.min(picp_values),
        "PICP标准差": np.std(picp_values),
        "平均PINAW": np.mean(pinaw_values),
        "最大PINAW": np.max(pinaw_values),
        "最小PINAW": np.min(pinaw_values),
        "PINAW标准差": np.std(pinaw_values),
        "各预测期指标": {
            f"{days}天": {
                "PICP": picp,
                "PINAW": pinaw
            }
            for days, picp, pinaw in zip(
                [days for days, _ in prediction_periods],
                picps,
                pinaws
            )
        }
    ...
    # 保存汇总报告
    summary_file = os.path.join(interval_dir, f"{station_name}_interval_summary.json")
    with open(summary_file, 'w', encoding='utf-8') as f:
        json.dump(summary, f, indent=2, ensure_ascii=False)
        
    # 输出汇总信息
    logger.info(f"\n站点{station_name}区间预测评估完成:")
    logger.info(f"目标覆盖率: {(1-alpha)*100:.0f}%")
    logger.info(f"平均PICP: {summary['平均PICP']:.4f}")
    logger.info(f"平均PINAW: {summary['平均PINAW']:.4f}")
    logger.info(f"区间评估报告保存在: {interval_dir}")
...
def process_station_with_vmd_optimization(station_name, upstream_stations, 
                                         nrbo_population=30, nrbo_iterations=50, alpha=0.05):
    ...
    try:
        ...
        for upstream_station in upstream_stations:
            ...
            if upstream_station in processed_stations_cache['denoised_runoff'] and upstream_station in processed_stations_cache['vmd_params']:
                ...
                if upstream_runoff_data is not None:
                    ...
                    logger.info(f"{upstream_station}站点最优VMD参数: K={K}, alpha={alpha_vmd:.2f}, tau={tau:.4f}")
        ...
        # 评估模型
        logger.info("\n开始模型评估...")
        predictions, actual_values, metrics = evaluate_predictions(
...
def process_all_stations_with_optimization(nrbo_population=30, nrbo_iterations=50, alpha=0.05):
    """处理所有站点的预测任务，包括VMD参数优化和区间预测
    
    Args:
        nrbo_population: NRBO算法种群大小
        nrbo_iterations: NRBO算法迭代次数
        alpha: 预测区间显著性水平，默认0.05 (95%预测区间)
    """
    # 建立站点上下游关系
    station_locations = {
    ...
    summary = {
        station: {
            ...
            "PICP": metrics["总体评估指标"]["PICP"],
            "PINAW": metrics["总体评估指标"]["PINAW"],
...
if __name__ == "__main__":
    try:
        ...
        # 询问用户是否使用区间预测
        use_interval = input("是否使用区间预测? (y/n): ").lower() == 'y'
        ...
        if user_input.lower() == 'y':
            ...
            # 如果启用了区间预测，为所有站点生成区间预测评估报告
            if use_interval:
                logger.info("\n开始生成所有站点的区间预测评估报告...")
                interval_summaries = {}
                for station in results.keys():
                    interval_summary = evaluate_interval_forecasts(station, alpha)
                    if interval_summary:
                        interval_summaries[station] = interval_summary
                
                # 保存所有站点的区间预测汇总
                with open("all_stations_interval_summary.json", "w", encoding="utf-8") as f:
                    json.dump(interval_summaries, f, indent=2, ensure_ascii=False)
            ...
            # 询问是否评估特定站点的区间预测
            if use_interval:
                station_eval = input("请输入要评估区间预测的站点名称 (留空跳过): ")
                if station_eval and station_eval in STATIONS:
                    evaluate_interval_forecasts(station_eval, alpha)
                    logger.info("单站点区间预测评估完成")
                else:
                    logger.info("跳过区间预测评估")
